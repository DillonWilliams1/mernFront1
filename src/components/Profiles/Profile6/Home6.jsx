...
function Home6() {
  ...
  useEffect(() => {
    axios
      .get('https://mernback1-n2ty.onrender.com/profile6')
  ...
  const handleDelete = (id) => {
    axios
    .delete(`https://mernback1-n2ty.onrender.com/profile6/delete/${id}`)
    .then((res) => {
      console.log(res);
  ...
  const handleCheckboxChange = (foodId) => {
    ...
    if (!isSelected) {
      ...
      axios.post('https://mernback1-n2ty.onrender.com/profile6/saveSelectedFood', selectedFoodDetails)
        .then((res) => {
      ...
      if (storedObjectId) {
        axios.delete(`https://mernback1-n2ty.onrender.com/profile6/deleteSelectedFood/${storedObjectId}`)
          .then((res) => {
  ...
  const handleClearTable = () => {
    axios.delete('https://mernback1-n2ty.onrender.com/profile6/clearTable')
      .then((res) => {
  ...
  <div className="w-75 bg-white rounded p-3">
    <div className="d-flex justify-content-between mb-3">
      <Link to="/profile6/create" className="btn btn-success">
        Add+
      </Link>
          ...
          <tbody>
            {foods.map((food) => (
              <tr key={food._id}>
                ...
                <td>{food.price}</td>
                <td>
                  <Link to={`/profile6/update/${food._id}`} className="btn btn-success">
                    Update
                  </Link>
                  <button
