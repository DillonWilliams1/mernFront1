...
function Home2() {
    ...
    useEffect(() => {
      axios
        .get('https://mernback1-n2ty.onrender.com/profile2')
    ...
    const handleDelete = (id) => {
      axios
      .delete(`https://mernback1-n2ty.onrender.com/profile2/delete/${id}`)
      .then((res) => {
        console.log(res);
    ...
    const handleCheckboxChange = (foodId) => {
      ...
      if (!isSelected) {
        ...
        axios.post('https://mernback1-n2ty.onrender.com/profile2/saveSelectedFood', selectedFoodDetails)
          .then((res) => {
            const newObjectId = res.data._id; // Assuming the ID is returned in the response
            console.log('Newly created object ID:', newObjectId);
        ...
        if (storedObjectId) {
          axios.delete(`https://mernback1-n2ty.onrender.com/profile2/deleteSelectedFood/${storedObjectId}`)
            .then((res) => {
    ...
    const handleClearTable = () => {
      axios.delete('https://mernback1-n2ty.onrender.com/profile2/clearTable')
        .then((res) => {
  ...
  <button className="btn btn-active btn-accent" onClick={handleLogout}>LOGOUT</button>
    ...
    <div className="w-75 bg-white rounded p-3">
      <div className="d-flex justify-content-between mb-3">
        <Link to="/profile2/create" className="btn btn-success">
          Add+
        </Link>
            ...
            <tbody>
              {foods.map((food) => (
                <tr key={food._id}>
                  ...
                  <td>{food.price}</td>
                  <td>
                    <Link to={`/profile2/update/${food._id}`} className="btn btn-success">
                      Update
                    </Link>
                    <button
